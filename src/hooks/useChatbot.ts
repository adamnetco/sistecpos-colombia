...
export function useChatbot(sourcePage: string) {
...
export function useChatbotVisibility(currentPath: string) {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (data) {
        const enabled = data.some((s: any) => currentPath === s.page_path || currentPath.startsWith(s.page_path + "/"));
        setVisible(enabled);
      }
    })();
  }, [currentPath]);
