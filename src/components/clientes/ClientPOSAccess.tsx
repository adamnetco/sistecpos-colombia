...
export function ClientPOSAccess() {
  ...
  const loadSavedSessions = async () => {
    ...
    try {
      const { data, error } = await supabase.rpc("get_client_pos_sessions", {
        _user_id: user.id,
      });
      if (!error && data) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (consent) {
      // Validate server-side and store if consented
      setLoading(true);
      try {
      ...
      // No consent: just open POS directly, no storage
      submitPOSForm(username, store, password);
        ...
        <Card className="border-2 border-primary/20 bg-primary/5">
          ...
          <CardContent className="space-y-2">
            {savedSessions.map((session) => (
              ...
              >
                <Button
                  onClick={() => handleQuickAccess(session)}
              ...
              <div className="flex items-start gap-3">
                ...
                <label htmlFor="consent" className="text-sm leading-snug cursor-pointer">
                  ...
                  <span className="text-muted-foreground block mt-0.5">
                    Tus datos se guardarán de forma encriptada para brindarte acceso rápido y soporte prioritario. Puedes revocar este permiso en cualquier momento.
                  </span>
                </label>
              </div>
