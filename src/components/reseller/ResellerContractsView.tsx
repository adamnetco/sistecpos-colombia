...
export default function ResellerContractsView() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (!user) return;
    // Contracts are linked via business_id, RLS handles access
    supabase
      .from("contracts")
      .select("*")
  ...
  if (loading) {
    return <div className="space-y-3">{[1, 2].map(i => <Skeleton key={i} className="h-24 w-full rounded-lg" />)}</div>;
  }
