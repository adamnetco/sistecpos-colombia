...
const articleTemplates: Record<string, Partial<DianArticleRow>> = {
  guia_paso_a_paso: {
    hero_icon: "FileCheck", hero_badge: "Guía Tutorial",
    sections: [
      ...
      { title: "La solución rápida: SistecPOS", content: "Con SistecPOS automatizas todo este proceso.", bullets: ["Certificado incluido", "Capacitación presencial", "Soporte técnico local"] },
    ],
    pain_vs_solution: [{ pain: "Proceso manual de X días", solution: "SistecPOS lo hace en horas" }],
    cta_text: "Simplifica tu facturación electrónica",
    cta_whatsapp_message: "Hola, quiero ayuda con facturación electrónica DIAN",
    faqs: [{ question: "¿Es obligatorio este trámite?", answer: "Sí, según la normativa vigente de la DIAN..." }],
    related_links: [{ label: "Facturación Electrónica", href: "/facturacion-electronica" }],
  },
  comparativa: {
    hero_icon: "Shield", hero_badge: "Comparativa",
    sections: [
      ...
      { title: "¿Por qué SistecPOS?", content: "SistecPOS ofrece la mejor relación precio-funcionalidad.", bullets: ["Facturación ilimitada", "16 módulos especializados", "Soporte presencial"] },
    ],
    pain_vs_solution: [{ pain: "Software costoso sin soporte local", solution: "SistecPOS desde $12 USD/mes con soporte presencial" }],
  ...
  problema_solucion: {
    hero_icon: "AlertTriangle", hero_badge: "Solución",
    sections: [
    ...
    pain_vs_solution: [{ pain: "Problema recurrente sin solución", solution: "SistecPOS lo resuelve automáticamente" }],
...
type FormState = Omit<DianArticleRow, "id" | "created_at" | "updated_at">;

const defaultForm: FormState = {
  slug: "", keyword: "", meta_title: "", meta_description: "",
  hero_icon: "FileText", hero_badge: "", h1: "", hero_subtitle: "",
  sections: [], pain_vs_solution: [], cta_text: "", cta_whatsapp_message: "",
  faqs: [], related_links: [], is_published: false, sort_order: 0, cluster: "otros",
};
...
export default function DianArticlesView() {
  ...
  const openEdit = (a: DianArticleRow) => {
    setEditing(a);
    setForm({
      slug: a.slug, keyword: a.keyword, meta_title: a.meta_title,
      meta_description: a.meta_description, hero_icon: a.hero_icon,
      hero_badge: a.hero_badge, h1: a.h1, hero_subtitle: a.hero_subtitle,
      sections: a.sections || [], pain_vs_solution: a.pain_vs_solution || [],
      cta_text: a.cta_text, cta_whatsapp_message: a.cta_whatsapp_message,
      faqs: a.faqs || [], related_links: a.related_links || [],
      is_published: a.is_published, sort_order: a.sort_order,
      cluster: a.cluster || "otros",
    });
    setDialogOpen(true);
  };
  ...
  const duplicateArticle = (a: DianArticleRow) => {
    setEditing(null);
    setForm({
      slug: a.slug + "-copia", keyword: a.keyword, meta_title: a.meta_title,
      meta_description: a.meta_description, hero_icon: a.hero_icon,
      hero_badge: a.hero_badge, h1: a.h1 + " (Copia)", hero_subtitle: a.hero_subtitle,
      sections: a.sections || [], pain_vs_solution: a.pain_vs_solution || [],
      cta_text: a.cta_text, cta_whatsapp_message: a.cta_whatsapp_message,
      faqs: a.faqs || [], related_links: a.related_links || [],
      is_published: false, sort_order: a.sort_order + 1,
      cluster: a.cluster || "otros",
    });
    setDialogOpen(true);
  };
  ...
  const updateSection = (idx: number, key: string, val: any) => {
    const updated = [...form.sections]; (updated[idx] as any)[key] = val; set("sections", updated);
  };
  ...
  const updatePVS = (idx: number, key: string, val: string) => {
    const updated = [...form.pain_vs_solution]; (updated[idx] as any)[key] = val; set("pain_vs_solution", updated);
  };
  ...
  const updateFaq = (idx: number, key: string, val: string) => {
    const updated = [...form.faqs]; (updated[idx] as any)[key] = val; set("faqs", updated);
  };
  ...
  const updateLink = (idx: number, key: string, val: string) => {
    const updated = [...form.related_links]; (updated[idx] as any)[key] = val; set("related_links", updated);
  };
    ...
    <div>
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Gestiona las guías del ecosistema DIAN con bloques dinámicos y clusters
          </p>
        </div>
      </div>
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          <DianImportButton existingSlugs={existingSlugs} />
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Configura contenido, SEO, cluster y previsualiza en tiempo real.</DialogDescription>
          </DialogHeader>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6">
            ...
            <div>
              <Accordion type="multiple" defaultValue={["meta", "hero", "sections"]} className="space-y-2">
                  ...
                  <AccordionContent className="space-y-3 pt-2">
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <Input value={form.keyword} onChange={(e) => set("keyword", e.target.value)} className="h-9" />
                      </div>
                    </div>
                    <div>
                      <Label className="text-xs">Meta Title (≤60)</Label>
                    ...
                    <div>
                      <Label className="text-xs">Meta Description (≤160)</Label>
                ...
                <AccordionItem value="hero">
                  <AccordionTrigger className="text-sm font-semibold">🎯 Hero & Cluster</AccordionTrigger>
                  <AccordionContent className="space-y-3 pt-2">
                    <div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      <div>
                        ...
                        <Input value={form.hero_badge} onChange={(e) => set("hero_badge", e.target.value)} className="h-9" />
                      </div>
                      <div>
                ...
                <AccordionItem value="sections">
                  <AccordionTrigger className="text-sm font-semibold">📄 Secciones ({form.sections.length})</AccordionTrigger>
                    ...
                    {form.sections.map((sec, idx) => (
                      <Card key={idx} className="border shadow-none">
                        <CardContent className="p-3 space-y-2">
                          ...
                          <Input value={sec.title} onChange={(e) => updateSection(idx, "title", e.target.value)} placeholder="Título" className="h-8 text-sm" />
                          <Textarea value={sec.content} onChange={(e) => updateSection(idx, "content", e.target.value)} placeholder="Contenido..." rows={3} className="text-sm" />
                          <div>
                            ...
                            <Textarea
                              ...
                              rows={3} className="text-xs font-mono"
                            />
                          </div>
                        </CardContent>
                      </Card>
                  ...
                  <AccordionContent className="space-y-3 pt-2">
                    {form.pain_vs_solution.map((item, idx) => (
                      <div key={idx} className="grid grid-cols-[1fr_1fr_auto] gap-2">
                        <Input value={item.pain} onChange={(e) => updatePVS(idx, "pain", e.target.value)} placeholder="Problema..." className="h-8 text-xs" />
                        <Input value={item.solution} onChange={(e) => updatePVS(idx, "solution", e.target.value)} placeholder="Solución..." className="h-8 text-xs" />
                        <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => removePVS(idx)}><Trash2 className="h-3 w-3 text-destructive" /></Button>
                      </div>
                    ))}
                    <Button variant="outline" size="sm" onClick={addPainVsSolution}><Plus className="h-3.5 w-3.5 mr-1" /> Agregar</Button>
                  </AccordionContent>
                </AccordionItem>
                      ...
                      <Card key={idx} className="border shadow-none">
                        <CardContent className="p-3 space-y-2">
                          <div className="flex justify-between">
                            ...
                            <Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => removeFaq(idx)}><Trash2 className="h-3 w-3 text-destructive" /></Button>
                          </div>
                          <Input value={faq.question} onChange={(e) => updateFaq(idx, "question", e.target.value)} placeholder="Pregunta..." className="h-8 text-sm" />
                          <Textarea value={faq.answer} onChange={(e) => updateFaq(idx, "answer", e.target.value)} placeholder="Respuesta..." rows={2} className="text-sm" />
                        </CardContent>
                      </Card>
                    ))}
                    <Button variant="outline" size="sm" onClick={addFaq}><Plus className="h-3.5 w-3.5 mr-1" /> Agregar FAQ</Button>
                  </AccordionContent>
                </AccordionItem>

                {/* RELATED LINKS */}
                <AccordionItem value="links">
                  <AccordionTrigger className="text-sm font-semibold">🔗 Enlaces ({form.related_links.length})</AccordionTrigger>
                  <AccordionContent className="space-y-3 pt-2">
                    {form.related_links.map((link, idx) => (
                      <div key={idx} className="grid grid-cols-[1fr_1fr_auto] gap-2">
                        <Input value={link.label} onChange={(e) => updateLink(idx, "label", e.target.value)} placeholder="Etiqueta..." className="h-8 text-xs" />
                        <Input value={link.href} onChange={(e) => updateLink(idx, "href", e.target.value)} placeholder="/guias-dian/..." className="h-8 text-xs" />
                        <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => removeLink(idx)}><Trash2 className="h-3 w-3 text-destructive" /></Button>
                      </div>
                    ))}
                    <Button variant="outline" size="sm" onClick={addRelatedLink}><Plus className="h-3.5 w-3.5 mr-1" /> Agregar Enlace</Button>
                  </AccordionContent>
                </AccordionItem>
              </Accordion>
            ...
            <div className="hidden lg:block">
              <Label className="text-xs text-muted-foreground mb-2 block">Vista previa en tiempo real</Label>
              <div className="sticky top-4">
                <DianArticlePreview
                  ...
                  pain_vs_solution={form.pain_vs_solution}
                  faqs={form.faqs}
          ...
          <DialogFooter>
            ...
            <Button onClick={() => saveMutation.mutate()} disabled={!form.slug || !form.h1 || saveMutation.isPending}>
