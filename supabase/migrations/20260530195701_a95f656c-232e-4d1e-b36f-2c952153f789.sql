...
-- External franchise panel lead import: new fields + upsert RPC
ALTER TABLE public.leads_trials
  ADD COLUMN IF NOT EXISTS external_lead_id integer,
  ADD COLUMN IF NOT EXISTS external_reseller_id integer,
  ADD COLUMN IF NOT EXISTS external_reseller_name text,
  ADD COLUMN IF NOT EXISTS external_store_id integer,
  ADD COLUMN IF NOT EXISTS pos_store_internal text,
  ADD COLUMN IF NOT EXISTS license_key_external text,
  ADD COLUMN IF NOT EXISTS pos_password_hash_external text,
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_leads_trials_external_lead_id
  ON public.leads_trials(external_lead_id) WHERE external_lead_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_leads_trials_external_token
  ON public.leads_trials(external_token) WHERE external_token IS NOT NULL;

-- Upsert RPC: receives the show_encuesta JSON exactly as the franchise panel emits.
CREATE OR REPLACE FUNCTION public.upsert_lead_from_external_json(_payload jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  _internal_status := CASE _status_ext
    ...
    ELSE 'active_trial'  -- llega con encuesta = ya hay demo
  END;

  -- day_demo "demo_day_14" → trial_ends_at
  ...
  IF _id IS NULL THEN
    INSERT INTO leads_trials (
      ...
      _internal_status, 'panel_franquiciado', _trial_end,
      ...
      'Importado desde Panel Franquiciado ('||COALESCE(_payload->>'reseller_name','')||')'
...
GRANT EXECUTE ON FUNCTION public.upsert_lead_from_external_json(jsonb) TO authenticated;
